...
# Set HERMES_ROOTLESS_PODMAN=1 for rootless Podman to skip user switch + chown and run as root inside.
if [ "$(id -u)" = "0" ] && [ "${HERMES_ROOTLESS_PODMAN:-}" != "1" ]; then
    ...
    # Skip chown entirely when HERMES_ROOTLESS_PODMAN=1 (rootless Podman).
    if [ "${HERMES_ROOTLESS_PODMAN:-}" != "1" ]; then
    ...
    # Skip when HERMES_ROOTLESS_PODMAN=1.
    if [ "${HERMES_ROOTLESS_PODMAN:-}" != "1" ] && [ -f "$HERMES_HOME/config.yaml" ]; then
